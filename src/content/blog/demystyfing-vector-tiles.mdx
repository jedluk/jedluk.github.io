...
well-known amount of tiles, which are populated with data specific to that level. On this level you can observe rounded 
continents boundaries with oceans (map generalization). Each next map zoom level contains 4 times more tiles (each tile have 
4 children). Such an approach is presented in the form of the diagram below.
...
to max zoom level but usually it's around 22-23. To be precise: I've already shown you how to set up your tile server, but it doesn't 
serve the purpose in production applications where global coverage and support for users worldwide are required. It's ideal for small-scale 
...
examples: level 4 - you will be able to see large rivers, 5 - large lakes, level 6: large roads, level 12: local city streets, level 13: buildings.
...
I'm using maptiler API to get single tile (no bias here, you can any other map tile provider). You can create new account (no card required) and use <a href="https://www.maptiler.com/cloud/pricing/" target="_blank" rel="noopener noreferrer">FREE plan</a>,
which gives you 100k free requests per month. Once you'll have an account you can get your own API key. Let's fetch single tile then and see how it looks from inside!
...
executed in the web worked (your main thread remains untouched, and the application runs smoothly).
```js
...
OK, job done! In Visual Studio Code you can see file content directly, by using <a href="https://marketplace.visualstudio.com/items?itemName=RandomFractalsInc.geo-data-viewer" target="_blank">Geo Data Viewer</a> plugin. Alternatively you can awesome web app, called <a href="https://geojson.io/" target="_blank" rel="noopener noreferrer">geojson.io</a>.
...
And so we've reached the end of this story. As always, I hope you enjoyed it!

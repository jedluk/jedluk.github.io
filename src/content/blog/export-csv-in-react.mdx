...
<strong>Comma-separated values (CSV)</strong> is a text file format that usually uses commas to separate values (although some applications 
...
<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/for...in#description" target="_blank" rel="noopener noreferrer">ECMA spec</a>
...
would be necessary in the case of fields containing a character that corresponds to the separator.
...
`URL.createObjectURL`. Such a string can be passed directly as `href` attribute.
...
<a href="https://www.npmjs.com/package/use-exportable-csv" target="_blank">use-exportable-csv</a>, which encapsulates entire procedure. 
...
so that link is created dynamically, just like in real world). When unomunting component link in unbound from a document.
